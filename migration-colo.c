...
static void *colo_thread(void *opaque)
{
    ...
    while (s->state == MIG_STATE_COLO) {
        ...
        colo_trans = qemu_bufopen("w", &colo_buffer);
        if (!colo_trans) {
...
void colo_process_incoming_checkpoints(QEMUFile *f)
{
    ...
    QEMUFile *ctl = NULL, *fb = NULL, *wb = NULL;
    int ret;
    uint64_t total_size;
    uint8_t *buf;
    ...
    while (true) {
        ...
        /* open colo buffer for write */
        wb = qemu_bufopen("w", &colo_buffer);
        if (!fb) {
            error_report("can't open colo buffer for write");
            goto out;
        }

        qemu_put_buffer(wb, buf, total_size);
        qemu_fclose(wb);
        g_free(buf);
